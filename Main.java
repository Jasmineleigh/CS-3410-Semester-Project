...
//		testShortDistance_StudentUnion_To_AshleyHall();
	...
	public static void findShortestPaths(Graph graph, int source, int n, int start, int destination, String priority) {
	     ...
	     if(priority.equals("Time")) {
	    	 ...
	    	 findBestPath(graph, source, n, start, destination);
	    	 return;
	    	 ...
	    	 for(Edge e: graph.getAdjacencyList().get(currentNode)) {
	    		 ...
	    		 if(!finalized[destinationNode] && 
	    			(listOfPriority.get(currentNode) + priorityParameter) < listOfPriority.get(destinationNode)) {
	    			 ...
	    			 queue.add((new Node(destinationNode, distances.get(destinationNode), 
	    					 times.get(destinationNode), ratings.get(destinationNode))));
	    		 }
	    	 }
	...
	private static void findBestPath(Graph graph, int source, int n, int start, int destination) {
		 List<Node> ratingsQueue = new ArrayList<>();
		 ratingsQueue.add(new Node(source, 0.0, 0.0, 0.0));
		 
		 PriorityQueue<Node> queue = new PriorityQueue<>(Comparator.comparingDouble(node -> node.getRating()));
		 queue.add(new Node(source, 0.0, 0, 0));
		 
		 List<Double> times = new ArrayList<>(Collections.nCopies(n, Double.MIN_VALUE));
	     List<Double> distances = new ArrayList<>(Collections.nCopies(n, Double.MIN_VALUE));
	     List<Double> ratings = new ArrayList<>(Collections.nCopies(n, Double.MIN_VALUE));
	     ...
	     while(!queue.isEmpty()) {
//	    	 ratingsQueue.sort(new RatingsComparator());
	    	 Node node = queue.poll();
	    	 ratingsQueue.remove(0);
	    	 int currentNode = node.getNode();
	    	 ...
	    	 for(Edge e: graph.getAdjacencyList().get(currentNode)) {
	    		 ...
	    		 if(!finalized[destinationNode] && 
	 	    		(ratings.get(currentNode) + rating) > ratings.get(destinationNode)) {
	    			 ratings.set(destinationNode, (ratings.get(currentNode) + rating));
	    			 predecessor[destinationNode] = currentNode;
//	    			 ratingsQueue.add((new Node(destinationNode, distances.get(destinationNode), 
//	    			 times.get(destinationNode), ratings.get(destinationNode))));
	    			 queue.add((new Node(destinationNode, distances.get(destinationNode), 
	    					 times.get(destinationNode), ratings.get(destinationNode))));
	     ...
	     for(int i = 0; i < n; i++) {
	    	 if(i != source && ratings.get(i) != Double.MIN_VALUE) {
